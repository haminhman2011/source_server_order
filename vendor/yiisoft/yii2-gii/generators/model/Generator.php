...
class Generator extends \yii\gii\Generator
{
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['tableName'], 'validateTableName', 'on' => 'default'],
            [['modelClass'], 'validateModelClass', 'skipOnEmpty' => false, 'on' => 'default'],
...
//        $excludeAttributes = ArrayHelper::compact(array_map('trim', explode(',', $this->skippedColumns)));
        foreach ($table->columns as $column) {
            if (in_array($column->name, $excludeAttributes, true)) {
                continue;
            }
    ...
    public function generateRules($table)
    {
        $types             = [];
        $lengths           = [];
        $excludeAttributes = ['is_delete', 'created_by', 'created_date', 'modified_date', 'modified_by'];
        foreach ($table->columns as $column) {
            if ($column->autoIncrement || in_array($column->name, $excludeAttributes, true)) {
                continue;
            }
	    ...
	    //typecasting flag to int
	    if (!is_bool( array_search( 'flag', array_keys( $table->columns ) ))) {
		    $rules[] = "['flag', 'filter', 'filter' => 'intval']";
	    }
