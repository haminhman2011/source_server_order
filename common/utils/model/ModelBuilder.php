...
class ModelBuilder
{
        ...
        if ($post && is_array($post)) {
            ...
            foreach ($post as $item) {
                if (isset($item['id']) && ! empty($item['id']) && (isset($multipleModels[$item['id']]) || ! empty($subModels))) {
                    ...
                    if ($item !== null) {
                        $models[] = new $modelClass(['attributes' => $item]);
                    }
                }
            }
        }
    ...
    public static function initSubModel(array $configs = array())
    {
        ...
        if ( ! empty($modelId)) {
            ...
            /** @var ActiveRecord[] $modelsAttributes */
            ...
            if ( ! empty($subModels)) {
                foreach ($subModels as $i => $subModelData) {
                    if ($modelsAttributes[$i]->isNewRecord) {
                        $modelsAttributes[$i]->load($subModelData);
                    } else {
                        $modelsAttributes[$i]->setAttributes($subModelData);
                    }
                }
            }
        ...
        \yii\base\Model::loadMultiple($modelsAttributes, Yii::$app->request->post());
