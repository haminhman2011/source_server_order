...
class Permission extends Component
{
    ...
    private function getListPermission()
    {
        ...
        if (Yii::$app->cache->exists('permission_' . $roleId)) {
            ...
            $role = Role::find()->equalTo('id', $roleId)->active()->select(['permissions'])->one();
            if (empty($role)) {
                $role = null;
            } else {
...
//        $roleIds     = Group::find()->equalTo('status', 1)->equalTo('user_id', $userId)->select(['role_id'])->createCommand()->queryColumn();
//        $permissions = [];
...
//            } else {
//                /** @var Role $role */
//                $role = Role::find()->active()->equalTo('id', $roleIds)->select(['permissions'])->one();
//                Yii::$app->cache->set('permission_' . $roleId, $role, 3600);
//            }
