...
$this->title                   = $module->isNewRecord ? 'Tạo module' : 'Cập nhật module';
?>
...
<form id="form_role">
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            <div class="form-group">
                <label for="txt_module_path">Module</label>
                <input type="text" class="form-control" id="txt_module_path">
            </div>
        </div>
        <div class="col-md-2 form-group">
    ...
    <div class="row">
        <div class="col-md-12 form-group">
            <table id="table_module_child" class="table table-striped table-bordered" style="width: 100%">
                <thead>
                <tr>
    ...
    $(function() {
        $('#form_role').on('submit', function() {
            if (Team.validate('form_role')) {
                ...
                Team.submitForm('<?= Url::to(['save-module']) ?>', formData);
            } else {
                $('.error').first().focus();
            }
            return false;
...
//		MODEL DETAIL
        ...
        $('#btn_add_module_child').on('click', function() {
            ...
            tableModuleChild.row.add([
                `<input name="ModuleChild[${index}][name]" value="${name}" class="form-control txt-name">`
                , `<input name="ModuleChild[${index}][controller]" value="${controller}" class="form-control txt-controller">`
                , `<input name="ModuleChild[${index}][role]" value="${role}" class="form-control txt-role">`
                , '<a class="btn btn-danger btn-remove-module-child"> <i class="glyphicon glyphicon-trash"></i> </a>']).draw(false);
        });
        ...
        body.on('click', '#btn_load_controller', function() {
            Team.blockUI();
            $.get('<?= Url::to(['load-controller']) ?>", {module: $("#txt_module_path").val()}, function(data) {
                ...
                for (let i = 0; i < json.length; i++) {
                    ...
                    tableModuleChild.row.add([
                        `<input name="ModuleChild[${index}][name]" value="${name}" class="form-control txt-name">`
                        , `<input name="ModuleChild[${index}][controller]" value="${name}" class="form-control txt-controller">`
                        , `<input name="ModuleChild[${index}][role]" value="${role}" class="form-control txt-role">`
                        , '<a class="btn btn-danger btn-remove-module-child"> <i class="glyphicon glyphicon-trash"></i> </a>']).draw(false);
                }
